  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-foreground">AI Route Recommendations</h1>
          ...
          return (
            <div
              ...
              className={cn(
                ...
                rec.status === "accepted" && "border-status-ok/30",
                rec.status === "rejected" && "border-status-critical/30 opacity-60",
                rec.status === "pending" && "border-kpi-border hover:border-primary/30"
              )}
            >
              <button
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {rec.status !== "pending" && (
                        <span className={cn(
                          ...
                          rec.status === "accepted" ? "bg-status-ok/20 text-status-ok" : "bg-status-critical/20 text-status-critical"
                        )}>
                          {rec.status}
                        </span>
                      )}
                ...
                <div className="border-t border-border px-4 py-4 space-y-3">
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                    <div className="rounded-md bg-secondary p-3">
                      <p className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Why</p>
                    ...
                    <div className="rounded-md bg-secondary p-3">
                      <p className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Expected Impact</p>
                  ...
                  {rec.status === "pending" && (
                    <div className="flex gap-2 pt-1">
                      <button
                        ...
                        className="flex items-center gap-1.5 rounded-md bg-status-ok/20 px-3 py-1.5 text-xs font-medium text-status-ok hover:bg-status-ok/30 transition-colors"
                      >
                        <Check className="h-3 w-3" /> Accept
                      </button>
                      <button
                        onClick={() => updateStatus(rec.id, "rejected")}
                        className="flex items-center gap-1.5 rounded-md bg-status-critical/20 px-3 py-1.5 text-xs font-medium text-status-critical hover:bg-status-critical/30 transition-colors"
                      >
                        <X className="h-3 w-3" /> Reject
                      </button>
                      <button className="flex items-center gap-1.5 rounded-md bg-secondary px-3 py-1.5 text-xs font-medium text-muted-foreground hover:text-foreground transition-colors">
